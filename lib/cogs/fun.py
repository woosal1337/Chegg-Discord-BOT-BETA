...
from lib.bot.firebaseauth import addUser, getUser, getUserMail, updateMail, deleteUser, addQuestion, updateQuestion, getQuestion, deleteQuestion
from lib.bot.saveMail import SaveMail
from datetime import datetime
from lib.bot.saveMail import find_question_id
...
class Fun(Cog):
    ...
    async def chegg_send_dm(self, ctx, link):

        userid = ctx.message.author.id
        if len(getQuestion(userid)) != 0:
            if str(ctx.message.author.id) == "623772185315639302" or str(ctx.message.author.id) == "618038532665114624":
                if "chegg.com" in link:
                    questionid = find_question_id(link)
